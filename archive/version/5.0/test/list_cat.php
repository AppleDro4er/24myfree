...
<?php
function add_edit($id=0,$title=""){
	echo "<form style=\"margin:75px;\" method=\"POST\" ENCTYPE=\"multipart/form-data\"><input type=\"hidden\" name=\"id\" value=\"$id\"><table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"><tr><td><b>Название:</b></td><td><input type=\"text\" name=\"title\" style=\"width:100%\" value=\"$title\"></td></tr>";
...
function show() {
	...
	if ($qur) {
		...
		if ($kol) {
			echo "<table align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"95%\" style=\"margin:25px 25px 25px 25px;\"><tr><td colspan=\"3\" align=\"right\"><b><a href=\"?des=add\">Добавить категорию</a></b></td></tr><tr><td align=\"center\"><b>Название (ID)</b></td><td align=\"right\"><b>Действия</b></td></tr>";
			while($rez = mysql_fetch_assoc($qur)) {
				echo "<tr>";
				echo "<td>".$rez["title"]." (".$rez["id"].")"."</td>";
				echo "<td align=\"right\"><a href=\"?des=edit&id=".$rez["id"]."\">РЕД.</a> | <a href=\"?des=del&id=".$rez["id"]."\">УДАЛИТЬ.</a></td></tr>";
			}
			echo "</table>";
		} else echo "<div align=\"right\">Не удается получить список новостей<br><b><a href=\"?des=add\">Добавить категорию</a></b></div>"; 
	} else echo "<font color=\"green\">Ошибка запроса</font>";
}

if(isset($_POST["add"])) {
	$title = mysql_real_escape_string(htmlspecialchars($_POST["title"]));
		$sql = "INSERT INTO catbl VALUES (0, \"$title\")";
		$qur = mysql_query($sql);
}		
if(isset($_POST["edit"])) {
	$title_t = mysql_real_escape_string(htmlspecialchars($_POST["title"]));
	$sql = "UPDATE catbl SET title=\"$title\" WHERE id = ".$_POST["id"]."";
	$qur = mysql_query($sql);
}
...
if(isset($_GET["des"])) {
	if($_GET["des"] == "edit"){
		...
		if($qur && $kol) {
			...
			echo add_edit($rez["id"],stripslashes($rez["title"]));
		}
