...
<li><a href="/admin/forms/games/">Игры</a></li>
<li><a href="/admin/forms/books/">Книги</a></li>
<li><a href="/admin/forms/hardware/">Железо</a></li>
<li><a href="/admin/forms/films/">Фильмы</a></li>
<li><a href="/admin/forms/anime/">Аниме</a></li>
<li><a href="/admin/forms/cat/">Добавить каталог</a></li></ul></nav>
<div id="content_wrap"><div class="widget_title"><a href="?des=add">
<h5>Добавить новость</h5></a></div><div class="widget_body">
<?php 
function add_edit($id_news=0,$id_cat=0,$id_author=0,$title_cat="",$id_raz=2,$title="",$description="",$full_date="",$img_name="",$img_half_name="",$img_half2_name="",$img_type="",$col="") {
...
$result=mysql_query("SELECT * FROM catbl");
if(mysql_num_rows($result) > 0){
	$cats = array();
	while($raz =  mysql_fetch_assoc($result)){
		$cats_ID[$raz['id_cat']][] = $raz;
		$cats[$raz['id_raz']][$raz['id_cat']] = $raz;
    }
}
function build_tree($cats,$parent_id,$only_parent){
	if(is_array($cats) and isset($cats[$parent_id])){
		foreach($cats[$parent_id] as $raz){
			...
			$tree .= '<option value="'.$id_cat.'" selected>'.$raz['title_cat'].' ['.$id_cat.']';
...
function show() {
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 2 ORDER BY newstbl.id_news");
...
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$text_news=$_POST["text_news"];$id_cat=$_POST["id_cat"];$id_raz=2;
...
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 2 ORDER BY newstbl.id_news DESC LIMIT 1");
$kol=mysql_num_rows($qur);if($qur&&$kol){while($rez = mysql_fetch_assoc($qur)){
mkdir($_SERVER["DOCUMENT_ROOT"]."/books/".$rez["title_cat"]."/".$rez["id_news"]."/",0777,true);
$text='<?php include($_SERVER["DOCUMENT_ROOT"]."/include/books.php");
...
<div class=\"SectionName\"><a href=\"/books/'.$rez["title_cat"].'/'.$rez["id_news"].'/\">'.$rez["title_cat"].'</a></div>
...
$fp=fopen($_SERVER["DOCUMENT_ROOT"]."/books/".$rez["title_cat"]."/".$rez["id_news"]."/index.php","w+");fwrite($fp,$text);fclose($fp);}}}}
if(isset($_POST["edit"])){
$uploaddir=$_SERVER["DOCUMENT_ROOT"].'/upload/';
$uploadfile=$uploaddir.basename($_FILES['userfile']['name']);
$img_name=basename($uploadfile);$img_half_name=basename($uploadfile);$img_half2_name=basename($uploadfile);
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$id_cat=$_POST["id_cat"];$id_raz=2;
...
if(isset($_POST["cancel"])){header('Location: /admin/forms/books/');}
if(isset($_GET["des"])){
if($_GET["des"]=="edit"){
...
$path=$_SERVER["DOCUMENT_ROOT"]."/books/".$rez["title_cat"]."/".$_GET["id_news"]."/";
removeDir($path);}}
