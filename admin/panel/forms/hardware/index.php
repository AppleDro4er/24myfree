...
<script src="/admin/panel/js/jquery.min.js"></script>
...
$date=$rez["last_login"];$t=date("H:i",$date);$Y=date("Y",$date);$d=date("d",$date);$m=date("m",$date);echo '<a href="/admin/panel/profiles/'.$id_user.'/">';
echo '<dd><a class="welcome_user" href="/admin/panel/profiles/'.$id_user.'/">'; ?>
...
<li><a href="/admin/panel/forms/games/"><span class="iconsweet">+</span>Игры</a></li>
<li><a href="/admin/panel/forms/books/"><span class="iconsweet">+</span>Книги</a></li>
<li><a href="/admin/panel/forms/hardware/"><span class="iconsweet">+</span>Железо</a></li>
<li><a href="/admin/panel/forms/films/"><span class="iconsweet">+</span>Фильмы</a></li>
<li><a href="/admin/panel/forms/anime/"><span class="iconsweet">+</span>Аниме</a></li></ul></nav>
...
<?php 
function add_edit($id_news=0,$id_cat=0,$id_author=0,$id_raz=3,$title="",$description="",$full_date="",$img_name="",$img_half_name="",$img_half2_name="",$img_type="",$col="") {
...
if($id_news){$qur=mysql_query("SELECT * FROM catbl WHERE id_raz=3");$kol=mysql_num_rows($qur);
...
$qur=mysql_query("SELECT * FROM catbl WHERE id_raz=3");$kol=mysql_num_rows($qur);
if($qur&&$kol){while($rez=mysql_fetch_assoc($qur)) {
...
function show() {
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 3 ORDER BY newstbl.id_news");
...
<a class="tip_north" original-title="User" href="/admin/panel/profiles/'.$id_author.'/">a</a>
...
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$text_news=$_POST["text_news"];$id_cat=$_POST["id_cat"];$id_raz=3;
...
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 3 ORDER BY newstbl.id_news DESC LIMIT 1");
$kol=mysql_num_rows($qur);if($qur&&$kol){while($rez = mysql_fetch_assoc($qur)){
mkdir($_SERVER["DOCUMENT_ROOT"]."/hardware/".$rez["title_cat"]."/".$rez["id_news"]."/",0777,true);
$text='<?php include($_SERVER["DOCUMENT_ROOT"]."/include/hardware.php");
...
<div class=\"SectionName\"><a href=\"/hardware/'.$rez["title_cat"].'/'.$rez["id_news"].'/\">'.$rez["title_cat"].'</a></div>
...
$fp=fopen($_SERVER["DOCUMENT_ROOT"]."/hardware/".$rez["title_cat"]."/".$rez["id_news"]."/index.php","w+");fwrite($fp,$text);fclose($fp);}}}}
if(isset($_POST["edit"])){
$uploaddir=$_SERVER["DOCUMENT_ROOT"].'/upload/';
$uploadfile=$uploaddir.basename($_FILES['userfile']['name']);
$img_name=basename($uploadfile);$img_half_name=basename($uploadfile);$img_half2_name=basename($uploadfile);
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$id_cat=$_POST["id_cat"];$id_raz=3;
...
if(isset($_POST["cancel"])){header('Location: /admin/panel/forms/hardware/');}
if(isset($_GET["des"])){
if($_GET["des"]=="edit"){
...
$path=$_SERVER["DOCUMENT_ROOT"]."/hardware/".$rez["title_cat"]."/".$_GET["id_news"]."/";
removeDir($path);}}
