...
$qur=mysql_query("SELECT * FROM catbl ORDER BY id_cat DESC");
...
$tree .= '<optgroup label="'.$rez['title_cat'].'">';
$tree .= build_tree($cats,$rez['id_cat'],true);
$tree .= '</optgroup>';}}
if($only_parent==true){
foreach($cats[$parent_id] as $rez){
$tree .= '<option value="'.$rez['id_cat'].'">'.$rez['title_cat'].' ['.$rez['id_cat'].']';
$tree .= build_tree($cats,$rez['id_cat'],false);
$tree .= '</option>';}}}
else return null;
return $tree;
...
function show() {
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user ORDER BY newstbl.id_news");
...
if(isset($_POST["cancel"])){header('Location: http://appledro4er.pw/admin/panel/forms/');}
if(isset($_GET["des"])){
if($_GET["des"]=="edit"){
