...
<?php 
function add_edit($id_news=0,$id_cat=0,$id_author=0,$title_cat="",$id_raz=1,$title="",$description="",$full_date="",$img_name="",$img_half_name="",$img_half2_name="",$img_type="",$col="") {
...
function show() {
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 1 ORDER BY newstbl.id_news");
...
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$text_news=$_POST["text_news"];$id_cat=$_POST["id_cat"];$id_raz=1;
...
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 1 ORDER BY newstbl.id_news DESC LIMIT 1");
$kol=mysql_num_rows($qur);if($qur&&$kol){while($rez = mysql_fetch_assoc($qur)){
mkdir($_SERVER["DOCUMENT_ROOT"]."/anime/".$rez["title_cat"]."/".$rez["id_news"]."/",0777,true);
$text='<?php include($_SERVER["DOCUMENT_ROOT"]."/include/anime.php");
...
<div class=\"SectionName\"><a href=\"/anime/'.$rez["title_cat"].'/'.$rez["id_news"].'/\">'.$rez["title_cat"].'</a></div>
...
$fp=fopen($_SERVER["DOCUMENT_ROOT"]."/anime/".$rez["title_cat"]."/".$rez["id_news"]."/index.php","w+");fwrite($fp,$text);fclose($fp);}}}}
if(isset($_POST["edit"])){
$uploaddir=$_SERVER["DOCUMENT_ROOT"].'/upload/';
$uploadfile=$uploaddir.basename($_FILES['userfile']['name']);
$img_name=basename($uploadfile);$img_half_name=basename($uploadfile);$img_half2_name=basename($uploadfile);
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$id_cat=$_POST["id_cat"];$id_raz=1;
...
if(isset($_POST["cancel"])){header('Location: /admin/forms/anime/');}
if(isset($_GET["des"])){
if($_GET["des"]=="edit"){
...
$path=$_SERVER["DOCUMENT_ROOT"]."/anime/".$rez["title_cat"]."/".$_GET["id_news"]."/";
removeDir($path);}}
