...
<li class="nav_dashboard active"><a href="/admin/">Главная</a></li>
...
<section id="main_content"><nav id="secondary_nav"><dl class="user_info"><dt>
<?php
...
?>
<img width="79px" src="" alt=""></a></dt><dd>
<?php echo '<a class="welcome_user" href="/admin/profiles/'.$id_user.'/">'; ?>
...
<?php 
function add_edit($id_news=0,$id_cat=0,$id_author=0,$id_raz=5,$title="",$description="",$full_date="",$img_name="",$img_half_name="",$img_half2_name="",$img_type="",$col="") {
...
if($id_news){$qur=mysql_query("SELECT * FROM catbl WHERE id_raz=5");$kol=mysql_num_rows($qur);
...
$qur=mysql_query("SELECT * FROM catbl WHERE id_raz=5");$kol=mysql_num_rows($qur);
if($qur&&$kol){while($rez=mysql_fetch_assoc($qur)) {
echo '<option value='.$rez["id_cat"].'>'.$rez["title_cat"].' ['.$rez["id_cat"].']</option>';}}}
echo '</select></td></tr>
...
function show() {
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 5 ORDER BY newstbl.id_news");
...
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$text_news=$_POST["text_news"];$id_cat=$_POST["id_cat"];$id_raz=5;
...
$qur=mysql_query("SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN raztbl ON newstbl.id_raz = raztbl.id_raz LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE newstbl.id_raz = 5 ORDER BY newstbl.id_news DESC LIMIT 1");
$kol=mysql_num_rows($qur);if($qur&&$kol){while($rez = mysql_fetch_assoc($qur)){
mkdir($_SERVER["DOCUMENT_ROOT"]."/games/".$rez["title_cat"]."/".$rez["id_news"]."/",0777,true);
$text='<?php include($_SERVER["DOCUMENT_ROOT"]."/include/games.php");
...
<div class=\"SectionName\"><a href=\"/games/'.$rez["title_cat"].'/'.$rez["id_news"].'/\">'.$rez["title_cat"].'</a></div>
...
$fp=fopen($_SERVER["DOCUMENT_ROOT"]."/games/".$rez["title_cat"]."/".$rez["id_news"]."/index.php","w+");fwrite($fp,$text);fclose($fp);}}}}
if(isset($_POST["edit"])){
$uploaddir=$_SERVER["DOCUMENT_ROOT"].'/upload/';
$uploadfile=$uploaddir.basename($_FILES['userfile']['name']);
$img_name=basename($uploadfile);$img_half_name=basename($uploadfile);$img_half2_name=basename($uploadfile);
$img_type=$_FILES['userfile']['type'];$title=$_POST["title"];$id_cat=$_POST["id_cat"];$id_raz=5;
...
if(isset($_POST["cancel"])){header('Location: /admin/forms/games/');}
if(isset($_GET["des"])){
if($_GET["des"]=="edit"){
...
$path=$_SERVER["DOCUMENT_ROOT"]."/games/".$rez["title_cat"]."/".$_GET["id_news"]."/";
removeDir($path);}}
