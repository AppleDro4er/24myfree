...
if(!ctype_digit($password1)){if(!ctype_alpha($password1)){if(!ctype_alnum($password1)){
if(!preg_match('/^(.)\1{3,}&/',$password1)){if($password1==$password2){
$recaptcha=$_POST['captcha_response'];if(!empty($recaptcha)){
require_once($_SERVER["DOCUMENT_ROOT"]."/login/getCurlData.php");
$url="https://www.google.com/recaptcha/api/siteverify?secret=6LfDRCgUAAAAAKDumvGZ7ExLwlXTvjiOe1JxxhjY&response=".$recaptcha."&remoteip=".$_SERVER['REMOTE_ADDR'];
