...
$(document).ready(function(){
  ...
  if($(window).scrollTop()+$(window).height()>=$(document).height()){
    if (next_page < totall_page) {
      ...
      $.ajax({
        type: "POST",
        url: '/index_ajax.php?page='+next_page,
        data: { FROM: 'hardware', SORT: 'new'},
...
function news() {
$sql = 'SELECT * FROM newstbl LEFT JOIN catbl ON newstbl.id_cat = catbl.id_cat LEFT JOIN usertbl ON newstbl.id_author = usertbl.id_user WHERE catbl.id_raz = 3 ORDER BY newstbl.id_news DESC LIMIT 6';
